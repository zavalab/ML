...
from pyomo.environ import (ConcreteModel, Constraint, Objective, Var, Param, value)
...
def getConcData(self,m):
    ...
    self.CAg = np.clip(np.array([value(m.CAg[t]) for t in m.t]),0,1)
    self.CBg = np.clip(np.array([value(m.CBg[t]) for t in m.t]),0,1)
    
    self.CA = np.clip(np.array([value(m.CA[t]) for t in m.t]),0,1)
    self.CB = np.clip(np.array([value(m.CB[t]) for t in m.t]),0,1)
