...
parser = argparse.ArgumentParser(description='2d fast with pc')
...
parser.add_argument('--im', default=25, type=int, metavar='N', help='2D image size')
...
def main(k, image_size, learning_rate, batch_size, model_checkpoint, num_filters, train):
    ...
    if image_size != 0:
        ...
        with open(data_dir + f'revalidate_fast_with_pc_{image_size}.pickle', 'rb') as handle:
            x_revalidate = pickle.load(handle)
            y_revalidate = pickle.load(handle)
        # concatenate the original and revalidation data.
        x = np.concatenate((x, x_revalidate), axis=0)
        y = np.concatenate([y, y_revalidate])
    # if image size is equal to 0, we load 1D CNN data.
    else:
        with open(data_dir + 'fast_with_pc.pickle', 'rb') as handle:
            x = pickle.load(handle)
            y = pickle.load(handle)
        with open(data_dir + 'revalidate_fast_with_pc.pickle', 'rb') as handle:
            x_revalidate = pickle.load(handle)
            y_revalidate = pickle.load(handle)
        # concatenate the original and revalidation data.
        x = np.concatenate((x, x_revalidate), axis=0)
    ...
    for train_index, test_index in skf.split(x, y):
        ...
        y_valid = keras.utils.to_categorical(y_valid, num_classes=14)
        y_test = keras.utils.to_categorical(y_test, num_classes=14)
        i += 1
        if i == k:
            print(f'This is fold {i}.')
            break
    # define the name for saving results
    name = f'revalidate_fast_wtih_pc_{k}_{image_size}_{learning_rate}_{batch_size}_{model_checkpoint}_{num_filters}'
