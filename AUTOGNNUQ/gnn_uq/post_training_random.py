...
def post_train(arch, name, dataset, seed, SPLIT_TYPE):
    ...
    """
    if not os.path.exists(os.path.join(ROOT_DIR, f'NEW_POST_RANDOM_MODEL/post_model_{dataset}_random_{seed}_split_{SPLIT_TYPE}/')):
        os.makedirs(os.path.join(ROOT_DIR, f'NEW_POST_RANDOM_MODEL/post_model_{dataset}_random_{seed}_split_{SPLIT_TYPE}/'))
    if not os.path.exists(os.path.join(ROOT_DIR, f'NEW_POST_RANDOM_RESULT/post_result_{dataset}_random_{seed}_split_{SPLIT_TYPE}/')):
        os.makedirs(os.path.join(ROOT_DIR, f'NEW_POST_RANDOM_RESULT/post_result_{dataset}_random_{seed}_split_{SPLIT_TYPE}/'))
    
    if not os.path.exists(os.path.join(ROOT_DIR, f"NEW_POST_RANDOM_RESULT/post_result_{dataset}_random_{seed}_split_{SPLIT_TYPE}/test_{name}.pickle")):
        ...
        cp = ModelCheckpoint(os.path.join(ROOT_DIR, f'NEW_POST_RANDOM_MODEL/post_model_{dataset}_random_{seed}_split_{SPLIT_TYPE}/best_{name}.h5'), monitor='val_loss',
        ...
        # make prediction
        model.load_weights(os.path.join(ROOT_DIR, f'NEW_POST_RANDOM_MODEL/post_model_{dataset}_random_{seed}_split_{SPLIT_TYPE}/best_{name}.h5'))
        ...
        with open(os.path.join(ROOT_DIR, f'NEW_POST_RANDOM_RESULT/post_result_{dataset}_random_{seed}_split_{SPLIT_TYPE}/test_{name}.pickle', 'wb')) as handle:
        ...
        with open(os.path.join(ROOT_DIR, f'NEW_POST_RANDOM_RESULT/post_result_{dataset}_random_{seed}_split_{SPLIT_TYPE}/val_{name}.pickle'), 'wb') as handle:
...
def main(dataset='delaney', topk=10, seed=0, SPLIT_TYPE="811"):
    ...
    for i in range(len(df)):
        loss_min_ = i #np.argsort(df['objective'])[::-1].values[i]
