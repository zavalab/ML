...
def compare_methods(filename_nsga , filename_bo ,
    ...
    for i in range(1, n_runs + 1):
        ...
        df["reboiler"] = df["reboiler"].round(2)

        df["distance"] = np.sqrt(
            (-1*df["pureza"] - pureza_opt)**2 +
            (df["reboiler"] - duty_opt)**2
        )
    ...
    # 2) BO (MOBO)
    # =========================
    file_base = 'LLExtraction_MOBO_STD'
    all_runs2 = []
    ...
    for i in range(1, n_runs + 1):
        ...
        df["pureza"]       = df["pureza"].round(4)
        df["reboilerduty"] = df["reboilerduty"].round(2)

        df["distance"] = np.sqrt(
            (df["pureza"] - pureza_opt)**2 +
            (-1*df["reboilerduty"] - duty_opt)**2
        )
        df["best_so_far"] = df["distance"].cummin()
