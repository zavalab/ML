# -*- coding: utf-8 -*-
"""
deep_cnn_solvent_net.py
Solven net is a 3D CNN to characterize MD simulation.
...
#%%
    ...
    x_train_xy = sp.ndimage.interpolation.rotate(x_train, 90, (1,2))
    x_train_xz = sp.ndimage.interpolation.rotate(x_train, 90, (1,3))
    x_train_yz = sp.ndimage.interpolation.rotate(x_train, 90, (2,3))
    x_train = np.concatenate((x_train, x_train_xy, x_train_xz, x_train_yz), axis=0)

#    x_test_xy = sp.ndimage.interpolation.rotate(x_test, 90, (1,2))
#    x_test_xz = sp.ndimage.interpolation.rotate(x_test, 90, (1,3))
#    x_test_yz = sp.ndimage.interpolation.rotate(x_test, 90, (2,3))
#    x_test = np.concatenate((x_test, x_test_xy, x_test_xz, x_test_yz), axis=0)
...
#    y_test = y_test.append(y_test)
    ...
    '''
    trained = 'no' # or 'no' if 'no' it will retrain the data
    ## CHANGE TO YOUR DIRECTORY TO STORE WEIGHTS
    filepath= r"C:\Users\jsl95\OneDrive - UW-Madison\Course\CS 760\Gridded_Data\Gridded_Data"+"\weights_stratified425_augmented2.best.hdf5"
...
#%%    '''PART IV: DATA ANALYSIS'''
    ...
    plt.legend(['train', 'test'], loc='upper left')
    plt.show()
    mean_squared_error2 = np.array(history.history['mean_squared_error'])
    val_mean_squared_error = np.array(history.history['val_mean_squared_error'])
#%%
    ## ACCURACY METRICS
