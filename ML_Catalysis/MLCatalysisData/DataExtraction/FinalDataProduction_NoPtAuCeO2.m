%{
    This script cleans the data and prepares it for direct input into the
    NN.  Final data set is the whole data set, excluding Pt/CeO2 and
    Au/CeO2. Data is saved in file named "final_withoutPtAuCeO2.txt". 
%}
...
%% -------------------------------------------------------------
% Calculate rate/(1-B) & Temperature Correction
%{
This block of code handles the temperature correction that was eventually
not used. This temperature correction was supposed to normalize the rates to
a single temperature (T_ref) using a single activation energy. 
%}
...
for i = 1: length(data_table.Rate_molsCO_min_molMetal_)
    ...
    CorrectionFactor = 1; % This line removes the temperature correction
...
min(data_table.FirstIE_supp)
max(data_table.FirstIE_supp)
...
%% Prepping table for input to the "final" NN
% Comment out the attributes we want to keep

data_table.Reference = [];
data_table.beta = [];
data_table.rate_for_adj = [];
% data_table.T_K = [];


% Control ANN Attributes/Inputs. 
% Comment the line with the attirbute you wish to keep

% Primary Metal Attributes ----
% data_table.BE_C = [];
data_table.BE_CO = [];
data_table.BE_O = [];
data_table.BE_H = [];
...
%data_table.MW_comp = [];
%data_table.Redox = [];
% data_table.FirstIE_supp = [];
% data_table.Electroneg_supp = [];
%data_table.OxidState_high_supp = [];
%data_table.OxidState_low_supp = [];

        
% Promoter Attributes ----------
%data_table.FirstIE = [];
%input_table.Electronegativity = [];
%data_table.CovalentRadius = [];
%data_table.ChargeLow = [];
%data_table.ChargeHigh = [];
% input_table.Z_IonicRad = [];
%data_table.Redox_Prom = [];
%data_table.MW_Prom = [];

% Normalize the data that is not already normalized  
% data_table.BE_C              = (data_table.BE_C-mean(data_table.BE_C))/std(data_table.BE_C);
% data_table.loading_base      = (data_table.loading_base - mean(data_table.loading_base)) / std(data_table.loading_base);
% data_table.Z_IonicRad        = (data_table.Z_IonicRad - mean(data_table.Z_IonicRad)) / std(data_table.Z_IonicRad);
% data_table.Electronegativity = (data_table.Electronegativity - mean(data_table.Electronegativity))/std(data_table.Electronegativity);
% data_table.loading_promoter  = (data_table.loading_promoter - mean(data_table.loading_promoter)) / std(data_table.loading_promoter);
% data_table.FirstIE_supp      = ( data_table.FirstIE_supp - mean(data_table.FirstIE_supp)) / std(data_table.FirstIE_supp);
% data_table.Electroneg_supp   = (data_table.Electroneg_supp - mean(data_table.Electroneg_supp)) / std(data_table.Electroneg_supp);
% data_table.CalcT_C           = (data_table.CalcT_C - mean(data_table.CalcT_C))/std(data_table.CalcT_C);
% data_table.CalcT_time        = (data_table.CalcT_time - mean(data_table.CalcT_time)) / std(data_table.CalcT_time);
% data_table.T_K               = (data_table.T_K - mean(data_table.T_K)) / std(data_table.T_K);
% data_table.H2Vol_            = (data_table.H2Vol_ - mean(data_table.H2Vol_)) / std(data_table.H2Vol_);
% data_table.COVol_            = (data_table.COVol_ - mean(data_table.COVol_))/std(data_table.COVol_);
% data_table.H2OVol_           = (data_table.H2OVol_ - mean(data_table.H2OVol_))/std(data_table.H2OVol_);
% data_table.CO2Vol_           = (data_table.CO2Vol_ - mean(data_table.CO2Vol_))/std(data_table.CO2Vol_);
% data_table.TOS_min_          = ( data_table.TOS_min_ - mean(data_table.TOS_min_))/std(data_table.TOS_min_);
% data_table.F_W               = (data_table.F_W - mean(data_table.F_W)) / std(data_table.F_W);
% 

writetable(data_table,'final_withoutPtAuCeO2_notNormalized.txt')

% max = max(data_table.rate_for_adj_norm);
