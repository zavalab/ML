%{
    This script will only keep the data for the Pt/CeO2 and Au/CeO2
    catalysts. All other data is discarded. The retained data will be
    cleaned consistently with the final data set. Therefore, it can be used
    in the final ANN. This data is saved in the file
    "final_onlyPtAuCeO2.txt"
%}
...
% Only Keep Au / CeO2 data and Pt / CeO2 data

    % Remove any Au / CeO2 data
    toDelete = data_table.Au ~= 0 & data_table.CeO2 ~= 0;
    new_data_table = data_table(toDelete,:);
   
    % Remove any Pt / CeO2 data
    toDelete = data_table.Pt ~= 0 & data_table.CeO2 ~= 0;
    new_data_table = vertcat(new_data_table, data_table(toDelete,:));
...
%% SYNTHESIS --------------------------------------------------
% -------------------------------------------------------------
% Make data set match other data sets
    ...
    toDelete = data_table.UGC ~= 0;
    data_table(toDelete,:) = [];
    data_table.UGC = [];
    
    toDelete = data_table.SCT ~= 0;
    data_table(toDelete,:) = [];
    data_table.SCT = [];
    
    toDelete = data_table.ME ~= 0;
    data_table(toDelete,:) = [];
    data_table.ME = [];
...
% Normalize the data that is not already normalized between 0 and 1 or -1
% and 1. 
...
writetable(data_table,'final_onlyPtAuCeO2.txt')
