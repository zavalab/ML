...
class solvent_dataset_binary:
    ...
    def plot_VLE(self,model,chemical_list,smiles_list,solv1_x_range,temperature=298.,
        ...
        return plot_data
    def plot_gamma(self,chemical_list,plot_data,save_figure=True,overlay_with_aspen=True,
...
class solvent_dataset_ternary:
    ...
    def __init__(self, input_file_path = "../data/output_ternary_all.csv",
                 solvent_list_path = "../data/solvent_list.csv",
                 generate_all=False,
                 return_hbond=True, 
                 return_comp=True, 
    ...
    def generate_solvsys(self,batch_size=5):
        ...
        solvsys.add_edges(torch.arange(n_solv*batch_size),torch.arange(n_solv*batch_size))    
        return solvsys
    def predict(self,idx,model,empty_solvsys):
        sample = self.__getitem__(idx).copy()
        sample['intra_hb1'] = torch.tensor(sample['intra_hb1']).unsqueeze(0)
        sample['intra_hb2'] = torch.tensor(sample['intra_hb2']).unsqueeze(0)
    ...
    def plot_VLE(self,model,chemical_list,smiles_list,
        ...
        if save_figure:
            print('---start saving figure---')
            plt.savefig('../analysis/pxy_{}_{}_{}.svg'.format(chemical_list[0].lower(),
                                                           chemical_list[1].lower(),
                                                           chemical_list[2].lower()),
                        pad_inches=0,dpi=400,transparent=True)
        ternary.plt.show()
        if save_data:
            print('---start saving data---')
            plot_data.to_csv("../analysis/pxy_{}_{}_{}.csv".format(chemical_list[0].lower(),
                                                                chemical_list[1].lower(),
                                                                chemical_list[2].lower()),
                                                                index=False)
        print('---overall finished in {:.2f} seconds---'.format(time.time()-start_time))
        return plot_data
    def replot_VLE(self,chemical_list,plot_data,save_figure=False,
        ...
        if save_figure:
            print('---start saving figure---')
            plt.savefig('../analysis/pxy_{}_{}_{}_replot.svg'.format(chemical_list[0].lower(),
                                                           chemical_list[1].lower(),
                                                           chemical_list[2].lower()),
                        pad_inches=0,dpi=400,transparent=True)
        ternary.plt.show()
